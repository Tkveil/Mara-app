...
// ── API — calls our own backend, never exposes API key ────────────
...
// ── PROMPTS ───────────────────────────────────────────────────────
const buildSystem = (p) =>
  `You are Mara — a sobriety companion. Calm, grounded, real. Not a therapist, not a recovery program. Like a trusted friend who gets it.
...
Reference their substance and goals naturally. If crisis: "This sounds big — is there someone you can call right now?"`;
...
export default function Home() {
  ...
  async function enableNotif() {
    ...
    else alert("Permission denied — enable in your browser settings.");
  }
      ...
      <div style={S.setupWrap}>
        ...
        <div style={S.card}>
          ...
          {step === 1 && <>
            ...
            <Row>
              <GBtn onClick={() => setStep(0)}>← Back</GBtn>
              <Btn onClick={() => scf.trim() && setStep(2)} dim={!scf.trim()} style={{ flex: 1 }}>Next →</Btn>
            </Row>
          </>}
          {step === 2 && <>
    ...
    <Page title="Mara — Chat">
      <div style={S.chatOuter}>
        <div style={S.chatHead}>
          <button onClick={() => { setScreen("home"); setMsgs([]); }} style={S.back}>← back</button>
        ...
        <div style={S.msgList}>
          ...
          {busy && (
            <div style={S.aRow}>
              <Av size={28} fs={13} />
              <div style={S.aBub}><span style={{ letterSpacing: 4, color: G }}>· · ·</span></div>
            </div>
          )}
